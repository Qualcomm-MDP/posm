...
def shoot_rays_for_image(
    ...
    for i in range(0, width, interval):
        ...
        while height_offset <= int(height_center):
            ...
            height_offset = abs(int((focal_length_adj * math.sin(tilt)) / (denom if denom != 0 else 0.0001)))
    ...
    for i, column_ray in enumerate(rays):
        for j, ray in enumerate(column_ray):
            ...
            if len(locations) != 0:
                ...
                if hasattr(street_mesh.visual, 'vertex_colors') and street_mesh.visual.vertex_colors is not None:
...
def run_raycaster(ingestion_data: Dict[str, Any], combined_mesh_path: str = "combined.glb") -> Dict[str, Any]:
    """
