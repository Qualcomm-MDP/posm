...
def clean_polygon_latlon(poly: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    ...
    if len(cleaned) >= 2 and cleaned[0] == cleaned[-1]: cleaned = cleaned[:-1]
    if len(cleaned) >= 3 and cleaned[0] != cleaned[-1]: cleaned.append(cleaned[0])
    return cleaned
...
def point_in_polygon_with_holes(
    lat: float, lon: float, outer_ring_closed: List[Tuple[float, float]], inner_rings_closed: Optional[List[List[Tuple[float, float]]]] = None
) -> bool:
    if not outer_ring_closed or len(outer_ring_closed) < 4: return False
    if not point_in_polygon(lat, lon, outer_ring_closed): return False
    if inner_rings_closed:
        for hole in inner_rings_closed:
            if hole and len(hole) >= 4 and point_in_polygon(lat, lon, hole): return False
    return True
...
def point_to_segment_distance_m(
    ...
    if ab2 <= 1e-12: return math.hypot(px - ax, py - ay), 0.0
